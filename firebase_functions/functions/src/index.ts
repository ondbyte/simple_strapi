...
exports.remindForBooking = functions.https.onRequest(async (req, resp) => {
    var reference = new DocumentReference(req.query.reference);


});


async function sendMessage(j:any){
    ...
    if(snaps.size == 1){
        ...
        var resp = await admin.messaging().sendToDevice(docData.fcmToken, j);
