...
class _LoginScreenState extends State<LoginScreen> {
  ...
  bool _askOTP = false;
  Completer<String> _otpFutureCompleter = Completer();
                  ...
                  onPressed: _canVerify
                      ? () {
                          authStore.loginOrSignUpWithNumber(
                            number: _number,
                            onAskOTP: () async {
                              _otpFutureCompleter = Completer<String>();
                              setState(() {
                                _askOTP = true;
                              });
                              return _otpFutureCompleter.future;
                            },
                            onFail: (e) {
                              Flushbar(
                                message: "${e.code}",
    ...
    return Padding(
      ...
      child: Center(
        child: Observer(builder: (_){
          return authStore.loadingForOTP? CircularProgressIndicator() : Column(
            ...
            children: [
              ...
              PrimaryButton("Verify",
                  onPressed: _canVerifyotp
                      ? () {
                    if (!_otpFutureCompleter.isCompleted) {
                      print(_otp);
                      _otpFutureCompleter.complete(_otp);
                    }
                  }
                      : null),
            ],
